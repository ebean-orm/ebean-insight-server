...
| `insight missing-plans [--app] [--env] [--by total\|mean\|max\|count] [--since-minutes N \| --since-hours N] [--older-than-minutes N \| --older-than-hours N] [--capture [--yes]] [-n N] [-i]` | Plan-capable metrics with no recent plan, ranked by cost. `--env` scopes the cost ranking to one environment (and targets it with `--capture`). `--capture` requests a plan for every listed row (capped by `-n`); `-i` drives an interactive drill-down. |
| `insight plans [--app] [--env] [--label] [--hash] [--since-minutes N] [--since-hours N] [-n/--limit N]` | List recently captured query plans (tabular). |
| `insight pending [--app] [--env] [--hash] [--label]` | List in-flight plan captures — requested but not yet collected (durable; shows AGE, ages out after ~15 min). |
